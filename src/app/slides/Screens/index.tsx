...
export const Screens = (props: any) => {
  ...
  const [isAddingCollection, setIsAddingCollection] = useState<boolean | null>(null)
  const [addToCollectionIndex, setAddToCollectionIndex] = useState<number | null>(null)
  ...
  const renderStorage = (storage: any) => {
                ...
                ? <div className='auto-scroll-y' style={{ position: 'relative', height: 'calc(100vh - 3rem)' }}>
                  ...
                  <Accordion isOpenDefault title={'Slideshows'}>
                    <div className='flex-row flex-wrap padded'>
                      {
                        (screen.slideIds || []).map((slideId: any, s: number) => (
                          <div
                            key={`${slideId}${s}`}
                            className='card'
                            style={{
